process runBraker4 {
    ...
    path "output/${params.species_id}/results/braker.gff3", emit: braker_annots
    path "output/${params.species_id}/results/braker.aa", emit: aa_seqs

    script:
    def rna_csv_cols
    if (params.bam) {
        def bam_string = rna_reads.collect { it.name }.join(':')
        rna_csv_cols = "${bam_string},,"   // bam_files,fastq_r1,fastq_r2
    } else {
        ...
        rna_csv_cols = ",${r1_string},${r2_string}"   // bam_files,fastq_r1,fastq_r2
    }
    ...
    echo "${params.species_id},${genome_assembly},${masked_assembly},${protein_ref},${rna_csv_cols},,,,,,${params.busco_lineage}," >> samples.csv
